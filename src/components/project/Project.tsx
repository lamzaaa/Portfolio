...
export default function SectionProjects() {
	return (
		<section className="section min-h-screen flex items-center" id="projects">
			<div className="container">
				<div className="text-center">
					<AnimateTitle
						title="Featured Projects"
						description="These are some highlight projects."
					/>
				</div>
...
function ProjectCard(
	...
	return (
		<motion.div
			...
			className="item px-5 py-6 bg-black hover:bg-primary/30 transition-colors"
		>
			...
			<Popup open={open} closeOnDocumentClick modal onClose={closeModal}>
				{
					...
					close => (
						...
						>
							<button
								className="close absolute -top-3 -right-3 size-10 bg-secondary rounded-sm [&_svg]:size-full"
								onClick={close}
							>
								<BaselineClose />
							</button>
							<div className="text-center text-lg -sm:font-bold sm:block-title">
								{project.title}
							</div>
							...
							<div className="content space-y-8">
								...
								{project.description && (
									<div className="description text-lg font-medium flex-1 [&_ul]:list-disc [&_ul]:pl-5 rounded-sm">
										<div className="label font-bold mb-1 border-b border-white mb-2 pb-2">
											Description
										</div>
										<div
											className="ctn line-clamp-3"
											dangerouslySetInnerHTML={{
